...
bn <- 3
A <- (shortest_paths_mat <= bn)*1; temp <- eigen(A);  A <- (temp$vectors)%*%diag((temp$values)*(temp$values>0))%*%solve(temp$vectors)
...
#第二套
file_path <- "/home/ZhihengZhang/ex_bn_3/ex_result/ex_spillover_Ap.txt"
# 将数据写入文本文件
write.table(res, file = "/home/ZhihengZhang/ex_bn_3/ex_result/ex_spillover_Ap.txt", col.names = FALSE)
# 提示数据已成功写入文件
cat("数据已成功写入文本文件:", file_path, "\n")
