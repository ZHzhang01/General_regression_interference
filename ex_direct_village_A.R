...
# # D_2 <- scale(X_db*w, scale = FALSE)
# D_2 <- X_db * w
# # hbeta_2 <- solve(t(D_2)%*%A%*%(D_2),   t(D_2)%*%A%*%(D-Leung)) #here we should use the new variance estimator:
# hbeta_2 <- solve(t(D_2)%*%A%*%(D_2),   t(D_2)%*%A%*%(D-  (mean(Y*w_haj_1)*w_1-mean(Y*w_haj_0)*w_0) )) 
...
quadratic_function <- function(hbeta) {
  ...
  # result <- t(D- D_2%*%hbeta  - Gao_F) %*% A %*% (D- D_2%*%hbeta  -Gao_F )/n^2 %>% as.vector()
  result<- sqrt(result)
  return(result)
}
# 使用优化函数找到最小值和对应的位置
