...
# }
# print(G)
# 假设您有一个 n*n 的非负整数矩阵 matrix_data
...
# pscore0 <- pbinom(0 ,size = rowSums(ex_adj_matrix), prob = 0.25);
...
mom_mat <- matrix(0, nrow = n, ncol =  1 + 2+2*ncol(X) )
for(i in 1:100){
  Z <- rbinom(nrow(X0422analysis), size = 1, prob = 0.25)
  X0422analysis$tempt_Z <- Z
  TZ <- get_T(Z)
...
#######################################################################################加上Lin's method
#we also need to compute the iteration debiasing procedure: we rewrite it:
# mom_mat <- matrix(0, nrow = n, ncol = 1+1 + 2+2*ncol(X))
# for(i in 1:100){
#   Z <- rbinom(nrow(X0422analysis), size = 1, prob = 0.25)
#   X0422analysis$tempt_Z <- Z
...
# 使用优化函数找到最小值和对应的位置
result <- optim( vector("logical", length = (2+2 * ncol(X))*2    ) * 1,          quadratic_function)
# 输出最优值和最优位置
...
# return(tibble(Leung, Gau_naive, Gao_F, Gao_L, Ours_G_ht_plus , Ours_G_haj_plus, var_Leung, var_Gao_naive_plus, var_Gao_F_plus,  var_Gao_L_plus,  var_Ours_G_ht_plus,  var_Ours_G_haj_plus))
# })
...
res <- tibble(Leung, Gau_naive, Gao_F, Gao_L, Ours_X_ht_plus, Ours_X_haj_plus, Ours_G_ht_plus , Ours_G_haj_plus, Ours_X_haj_plus_lin,Ours_G_haj_plus_lin, var_Leung, var_Gao_naive_plus, var_Gao_F_plus,  var_Gao_L_plus,  
              var_Ours_X_ht_plus, var_Ours_X_haj_plus, var_Ours_G_ht_plus,  var_Ours_G_haj_plus, var_Ours_X_haj_plus_lin, var_Ours_G_haj_plus_lin)
