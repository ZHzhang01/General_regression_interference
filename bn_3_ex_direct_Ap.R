load("/home/ZhihengZhang/703.RData")
...
bn <- 3
A <- (shortest_paths_mat <= bn); temp <- eigen(A);  A <- (temp$vectors)%*%diag((temp$values)*(temp$values >= 0))%*%solve(temp$vectors) 
...
# A <- ((ex_adj_matrix)>0)*1; temp <- eigen(A);  A <- (temp$vectors)%*%diag((temp$values)*(temp$values>0))%*%solve(temp$vectors)
...
for(i in 1:10000){
  ...
  X_aug <- cbind(X, get_rate(Z), get_rate_2(Z), Z_sub) #in each simulation, we need compute the new $X_aug$ (n*4);
...
# var_Ours_X_haj_plus <- t(D- D_2%*%hbeta_2_haj_5  -(mean(Y*w_haj_1)*w_1-mean(Y*w_haj_0)*w_0) ) %*%A %*% (D- D_2%*%hbeta_2_haj_5  -(mean(Y*w_haj_1)*w_1-mean(Y*w_haj_0)*w_0) )/n^2 %>% as.vector()
...
#第二套
file_path <- "/home/ZhihengZhang/ex_bn_3/ex_result/ex_direct_Ap.txt"
# 将数据写入文本文件
write.table(res, file = "/home/ZhihengZhang/ex_bn_3/ex_result/ex_direct_Ap.txt", col.names = FALSE)
# 提示数据已成功写入文件
cat("数据已成功写入文本文件:", file_path, "\n")
