...
def assemble_data():
    ...
    cols = ['UID','ID','SCHID','STRB','WRISTOW2','TREAT','SCHTREAT']+['ST'+str(i) for i in range(1,11)]+['GENDER'] + ['GRADESWELLWORKWITH']+['GRADE_LEVEL']
    ...
    data = data[data.GRADESWELLWORKWITH >= 0]
    data = data[data.GRADE_LEVEL>=0]
...
#     data = data[(data.SCHID == 24, 56, 22, 60, 58)] # restrict sample to five largest schools
    ...
    # X = [data.GENDER.values, data.GRADESWELLWORKWITH.values, data.GRADE_LEVEL.values]
    X = data.GENDER.values
    # X[X== -98 ] = np.zeros(X.size)[X==0.5]   # recode unkonwn gender to 0.5
    ...
    return Y, X, D,A,A_norm,pscores0,IDs


missing_values = ['--blank--','--impossible--','--shifted--','--nom--','--void--','-55','-66','-77','-88','-95','-96','-97','-98','-99','999',' ']
# # cols = ['UID','ID','SCHID','STRB','WRISTOW2','TREAT','SCHTREAT']+['ST'+str(i) for i in range(1,11)]+['GENDER']
