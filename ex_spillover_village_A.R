...
# 使用优化函数找到最小值和对应的位置
result0 <- optim(  vector("logical", length = ncol(D_2)   ) * 1    ,    quadratic_function, control = control)
...
#################################尝试使用新的G: (X transform to our_G)
treat1 <- filtered_data$ex_intensive
#first case
X_aug_new <- get_X(ourG,treat1, G ) #we revise it!
# X_aug <- cbind(X_aug, 1)
X_db_new <- X_aug_new - w_haj * (orth_coef_haj_new)  #it is n*4;  n*1,  n*4
# D_2 <- scale(X_db*w, scale = FALSE)
D_2 <- X_db_new * w
quadratic_function <- function(hbeta) {
  result <- t(D- D_2%*%hbeta  -(mean(Y*w_haj_1)*w_1-mean(Y*w_haj_0)*w_0) ) %*% A %*% (D- D_2%*%hbeta  -(mean(Y*w_haj_1)*w_1-mean(Y*w_haj_0)*w_0) )/n^2 %>% as.vector()
...
# 使用优化函数找到最小值和对应的位置
result0 <- optim(  vector("logical", length = ncol(D_2)   ) * 1    ,    quadratic_function, control = control)
result1 <- optim( matrix(c(0,0, beta_gao_f,0, rep(0, ncol(ourG)))) ,         quadratic_function)
# 输出最优值和最优位置
cat("最小值：", min(result0$value,result1$value) , "\n")
cat("最优位置：", result0$par, "\n")
hbeta_2_haj <- result0$par
...
X_aug_new <- get_X(ourG,treat1, G ) #we revise it!
# X_aug <- cbind(X_aug, 1)
X_db_new <- cbind(X_aug_new * T_vec, X_aug_new * (1-T_vec))
X_db_new <- X_db_new - w_haj * (orth_coef_haj_lin_new)  #it is n*4;  n*1,  n*4
# D_2 <- scale(X_db*w, scale = FALSE)
D_2 <- X_db_new * w
quadratic_function <- function(hbeta) {
  result <- t(D- D_2%*%hbeta  -(mean(Y*w_haj_1)*w_1-mean(Y*w_haj_0)*w_0) ) %*% A %*% (D- D_2%*%hbeta  -(mean(Y*w_haj_1)*w_1-mean(Y*w_haj_0)*w_0) )/n^2 %>% as.vector()
